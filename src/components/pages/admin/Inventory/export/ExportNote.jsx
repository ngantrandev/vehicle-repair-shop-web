...
import ExportNoteList from './ExportNoteList';
...
function ExportNote() {
    ...
    useEffect(() => {
        if (timeRange) {
            ...
            notes.forEach((element) => {
                if (
                    element.date_output >= startDate &&
                    element.date_output <= endDate
                ) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await adminInventoryService.getExportNotes();
            ...
            <div className='mb-8 w-full overflow-hidden rounded-2xl pt-0 shadow-[rgba(0,5,0,0.15)_1px_1px_60px_1px]'>
                ...
                <div className='grid w-full grid-cols-6 gap-4 p-8'>
                    <div className='col-span-2 col-start-1 flex flex-col'>
                        <label htmlFor='time'>Ngày xuất</label>
                        <RangePicker
                            value={timeRange}
